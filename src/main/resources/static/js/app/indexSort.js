export const sortPosts = {
    ...
    updatePagination: function(pagingData, sort) {
        ...
        for(let i = 0; i < pagingData.totalPages; i++) {
            this.createPaginationButton(i === pagingData.number, i, i + 1, sort, paginationContainer);
        }

        this.createPaginationButton(pagingData.last, pagingData.number + 1, '다음', sort, paginationContainer);

        const paginationParent = document.querySelector('#page')

    },

    createPaginationButton: function (isDisabled, pageNumber, text, sort, container) {
